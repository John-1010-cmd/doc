...
tags: 
  - Redis
  - 原理
description: 深入 Redis 单线程事件驱动模型，解析文件事件、时间事件及 aeEventLoop 的实现原理。
series: Redis 实战指南
series_order: 2
---
...
![db-redis-event-1](db-redis-event-1.png)
...
![db-redis-event-2](db-redis-event-2.png)

如下图所示，文件事件处理器有四个组成部分，它们分别是`套接字、I/O多路复用程序、文件事件分派器、事件处理器`：

![db-redis-event-3](db-redis-event-3.png)
...
![db-redis-event-4](db-redis-event-4.png)
...
内核会一直监听这些套接字上的连接请求或数据请求。一旦有请求到达，就会交给 Redis 线程处理，这就**实现了一个 Redis 线程处理多个 I/O 流的效果。**

![db-redis-event-0](db-redis-event-0.jpg)
...
![db-redis-event-5](db-redis-event-5.png)
...
它的核心操作就是调用 **epoll** 的 **epoll_ctl** 函数来向 **epoll** 注册响应事件。
