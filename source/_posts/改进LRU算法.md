...
tags: 
  - MySQL
  - 原理
description: 分析 MySQL Buffer Pool 和操作系统对 LRU 算法的改进策略，理解冷热数据分离的设计思想。
series: 数据库技术详解
series_order: 4
---
...
![img](v2-16c98d67dd7243176749c53c7d66f002_720w.webp)

现在有个编号为 20 的页被预读了，这个页只会被插入到 inactive list 的头部，而 inactive list 末尾的页（10号）会被淘汰掉。

![img](v2-28757626afd30788207112ce15a92ac9_720w.webp)
...
![img](v2-e1c99769f6b234af6e6e9c7c89051de6_720w.webp)
...
![img](v2-4c604b9639f35824aab2692e08691a1e_720w.webp)

**划分这两个区域后，预读的页就只需要加入到 old 区域的头部，当页被真正访问的时候，才将页插入 young 区域的头部**。如果预读的页一直没有被访问，就会从 old 区域移除，这样就不会影响 young 区域中的热点数据。

![img](v2-7ba7ee9eb267c9d62e95c3e858419e32_720w.webp)

现在有个编号为 20 的页被预读了，这个页只会被插入到 old 区域头部，而 old 区域末尾的页（10号）会被淘汰掉。

![img](v2-b381ae4b18804701b12cad3b770167b2_720w.webp)

如果 20 号页一直不会被访问，它也没有占用到 young 区域的位置，而且还会比 young 区域的数据更早被淘汰出去。

如果 20 号页被预读后，立刻被访问了，那么就会将它插入到 young 区域的头部，young 区域末尾的页（7号），会被挤到 old 区域，作为 old 区域的头部，这个过程并不会有页被淘汰。

![img](v2-73eeea4b1a6b0e6f0553b3ef07e36e4a_720w.webp)
